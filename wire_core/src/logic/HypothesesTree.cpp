...
/* ****************************************************************************** */
    ...
    void HypothesisTree::expandTree(const EvidenceSet &ev_set) {
        ...
        for (EvidenceSet::const_iterator it_ev = ev_set.begin(); it_ev != ev_set.end(); ++it_ev) {
            // new
            new_assignments.push_back(new Assignment(Assignment::NEW, *it_ev, 0,
                                                     KnowledgeDatabase::getInstance().getProbabilityNew(**it_ev)));

            // clutter
            clutter_assignments.push_back(new Assignment(Assignment::CLUTTER, *it_ev, 0,
                                                         KnowledgeDatabase::getInstance().getProbabilityClutter(
                                                                 **it_ev)));
        }
        ...
        std::priority_queue < AssignmentSet * , std::vector < AssignmentSet * >, compareAssignmentSets >
                                                                                 assignment_sets;
        ...
        for (std::list<Hypothesis *>::iterator it_hyp = leafs_.begin(); it_hyp != leafs_.end(); ++it_hyp) {
            ...
            for (std::list<Assignment *>::iterator it_ass = new_assignments.begin();
                 it_ass != new_assignments.end(); ++it_ass) {
                hyp->addPotentialAssignment(*it_ass);
            }

            // add clutter assignments to current hypothesis
            for (std::list<Assignment *>::iterator it_ass = clutter_assignments.begin();
                 it_ass != clutter_assignments.end(); ++it_ass) {
                hyp->addPotentialAssignment(*it_ass);
            }
        ...
        while (!assignment_sets.empty() && leafs_.size() < num_max_hyps_ &&
               assignment_sets.top()->getProbability() > min_prob) {
            ...
            for (std::list<AssignmentSet *>::iterator it_child = child_assignment_sets.begin();
                 it_child != child_assignment_sets.end(); ++it_child) {
    ...
    void HypothesisTree::pruneTree(const Time &timestamp) {
        ...
        while (!hyp_stack.empty()) {
            ...
            if (!children.empty()) {
                ...
                for (std::list<Hypothesis *>::const_iterator it_child = children.begin();
